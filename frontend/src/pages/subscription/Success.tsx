...
const Success = () => {
  ...
  const handleDownloadReceipt = async () => {
    ...
    try {
      const response = await apiClient.get(`/subscribe/receipt?sessionId=${sessionId}`);
      window.open(response.data.url, '_blank');
    } catch (err) {
      console.error('Failed to get receipt:', err);
      alert('Your receipt is still being generated by Stripe. Please check back in a moment or your email.');
    }
  };

  const handleShare = async () => {
    const shareData = {
      title: 'I just joined the Digital Clubhouse!',
      text: 'My golf game is now driving real-world change. Join the Fairway Fund community!',
    ...
    if (navigator.share) {
      ...
      alert('Success! Clubhouse link copied to clipboard. Share it with your network!');
    }
  };
        ...
        <div className="w-full max-w-5xl space-y-12">
          ...
          >
            <button 
              ...
              className="bg-white rounded-2xl p-6 flex items-center gap-5 border border-black/[0.03] shadow-sm hover:shadow-md transition-shadow group"
            >
              ...
              <div className="text-left">
                ...
                <p className="text-[11px] text-on-surface-variant/40 font-medium">Official Invoice (PDF)</p>
              </div>
            </button>
            <button 
              onClick={handleShare}
